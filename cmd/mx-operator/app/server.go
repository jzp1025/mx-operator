...
import (
	...
	"github.com/kubeflow/mx-operator/cmd/mx-operator/app/options"
	"github.com/kubeflow/mx-operator/pkg/apis/mxnet/v1alpha1"
	...
	"github.com/kubeflow/mx-operator/pkg/controller"
...
func Run(opt *options.ServerOption) error {
	...
	kubeClient, leaderElectionClient, mxJobClient, err := createClients(config)
	if err != nil {
		return err
	}
	...
	mxJobInformerFactory := informers.NewSharedInformerFactory(mxJobClient, time.Second*30)
	controller, err := controller.New(kubeClient, mxJobClient, *controllerConfig, mxJobInformerFactory, opt.EnableGangScheduling)
	if err != nil {
		return err
	}

	go mxJobInformerFactory.Start(neverStop)
...
func createClients(config *rest.Config) (clientset.Interface, clientset.Interface, mxjobclient.Interface, error) {
	kubeClient, err := clientset.NewForConfig(rest.AddUserAgent(config, "mxjob_operator"))
	if err != nil {
	...
	return kubeClient, leaderElectionClient, mxJobClient, nil
}
